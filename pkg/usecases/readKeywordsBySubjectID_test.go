...
func TestReadKeywordsBySubjectID(t *testing.T) {
	...
	scienceKeywords, err := readKeywordsBySubjectID.Exec(user.ID, scienceSubj.ID)
	if err != nil {
		t.Error(err)
	}

	mathKeywords, err := readKeywordsBySubjectID.Exec(user.ID, mathSubj.ID)
	if err != nil {
		t.Error(err)
	}
